...
int jzReadEndRecord(JZFile *zip, JZEndRecord *endRecord) {
    ...
    for(i = readBytes - sizeof_JZEndRecord; i >= 0; i--) { // need to + 2
...
int jzReadCentralDirectory(JZFile *zip, JZEndRecord *endRecord,
    ...
    for(i=0; i<endRecord->numEntries; i++) {
        int t = zip->tell(zip);
        if(zip->read(zip, &fileHeader, sizeof_JZGlobalFileHeader ) <
                sizeof_JZGlobalFileHeader) {
        ...
        memcpy(&header, &fileHeader.compressionMethod, sizeof_JZFileHeader);
        header.offset = *(uint32_t*)(fileHeader.relativeOffsetOflocalHeader);
...
int jzReadLocalFileHeaderRaw(JZFile *zip, JZLocalFileHeader *header,
    ...
    if(zip->read(zip, header, sizeof_JZLocalFileHeader) <
            sizeof_JZLocalFileHeader)
        return Z_ERRNO;
    ...
    if(header->compressionMethod == 0 &&
            ( *(uint32_t*) header->compressedSize != *(uint32_t*)header->uncompressedSize))
...
int jzReadLocalFileHeader(JZFile *zip, JZFileHeader *header,
    ...
    memcpy(header, &localHeader.compressionMethod, sizeof_JZFileHeader);
...
int jzReadData(JZFile *zip, JZFileHeader *header, void *buffer) {
    ...
    if(header->compressionMethod == 0) { // Store - just read it
        if(zip->read(zip, buffer, *(uint32_t*)header->uncompressedSize) <
            *(uint32_t*)header->uncompressedSize || zip->error(zip))
            return Z_ERRNO;
#ifdef HAVE_ZLIB
        ...
        unsigned long destlen = *(uint32_t*)header->uncompressedSize,
                      sourcelen = *(uint32_t*)header->compressedSize;
...
stdio_read_file_handle_read(JZFile *file, void *buf, size_t size)
{
    ...
    int r = fread(buf, 1, size, handle->fp);
    return r;
}
